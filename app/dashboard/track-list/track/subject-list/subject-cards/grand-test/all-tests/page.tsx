...
export default function AllGrandTestsPage() {
  ...
  const handleRowClick = (exam: GrandExam) => {
    ...
    sessionStorage.setItem("exam_id_grand_test", exam.id.toString());
                    ...
                    {pagedData.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="p-3 font-bold">
                          <div>{row.title}
                            <div className='text-xs text-slate-600'>{isPracticeExam ? 'Practice' : 'Grand'} Test</div>
                          </div>
                        </td>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => { if (!isUploading) { setIsUploadOpen(false); setSelectedFile(null); setUploadError(''); } }}>
            <div className="bg-white m-2 p-6 rounded shadow-lg w-full max-w-lg relative" onClick={(e) => e.stopPropagation()}>
              ...
              <div
                ...
                onDrop={handleDrop}
                onClick={() => document.getElementById('grand-test-upload-input')?.click()}
              >
                <input id="grand-test-upload-input" type="file" accept=".xlsx,.csv" className="hidden" onChange={(e) => { if (e.target.files) onFileChosen(e.target.files); }} />
